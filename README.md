...
ej8 - Para que el servidor permita aceptar conexiones y procesar mensajes en paralelo, se utilizó la biblioteca ```multiprocessing```. Cuando el servidor recibe una conexión, se levanta un proceso que se encarga de recibir los mensajes del cliente con la función ```self.__handle_client_connection```. Cuando el proceso termina de recibir todas las apuestas y tiene que guardarlas en el archivo, se utiliza un lock para que solo acceda un proceso a la vez. También se utiliza un manager para acceder a variables compartidas de forma segura, que son ```self._clients```, ```self._agencies```.
